...
'''
Ref: https://github.com/LiangjunFeng/Implement-of-ZSL-algorithms 
Relational Knowledge Transfer for Zero-Shot Learning
在此基础上我注释掉了一些无用的代码， 并添加了一些数据集使其可以跑通
'''
...
trainfeature_mean = trainfeatures_tabel.groupby('label').mean().values.T
# trainfeature_std = np.mat(trainfeatures_tabel.groupby('label').std().values).T


clf = Lasso(alpha=0.01)
clf.fit(train_attributetable.values.T,test_attributetable.values.T)
W = clf.coef_.T
